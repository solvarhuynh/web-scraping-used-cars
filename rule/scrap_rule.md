...
**Checkpointing Mechanism**: Never run the scraper continuously from the first page to the last in a single go. Save the last successfully scraped page number to a tracking file located in the raw data directory (e.g., `data/raw/checkpoint_{website_name}.txt`). If the process is interrupted, your script must automatically read this file and resume from the last saved page.
...
1.  **File & Directory Structure**:
    ...
    -   The output must be a CSV file saved to the `data/raw/` directory.
